...
const Main = (props) => {
  ...
  React.useEffect(() => {
    ...
    wrapperRef.current.addEventListener('click', (e) => {
      ...
      for (const item of categoryRef.current) {
        if (item.classList.contains('category-item-on'))
          item.classList.remove('category-item-on');
      }
      setCategory('random');
  ...
  const randomStart = () => {
    ...
    if (
      category !== 'random' &&
      small_category !== 'random' &&
      small_category !== 'ALL'
    ) {
      const _small_category = small_category
        .split('')
        .map((a) => (a === '/' ? (a = '%2F') : a))
        .join('');
    ...
    if (
      category !== 'random' &&
      (small_category === 'random' || small_category === 'ALL')
    ) {
              ...
              <div className='main-small-category-list'>
                {toefl_category.map((a, i) => {
                  return (
                    <div
                      key={i}
                      ref={(el) => (smallRef.current[i] = el)}
                      onClick={(e) => {
                        if (small_category !== e.target.innerText) {
                          setSmallCategory(e.target.innerText);
                          for (const item of smallRef.current) {
                            if (item?.classList.contains('small-category-on'))
                              item?.classList.remove('small-category-on');
                          }
                          smallRef.current[i]?.classList.add(
                            'small-category-on'
                          );
                        } else {
                          setSmallCategory('random');
